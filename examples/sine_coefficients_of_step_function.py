from hmod.hmod import compute_sine_base_coefficients_pw_constants
...
if __name__ == "__main__":
    ...
    K = int(100*nt)
    #sine_coeffs = compute_sine_base_coefficients_pw_constants(K, dof_vals, nt)

    #get the transformation as Linear operator
    Trans = LegendreToHilbertBase(K, nt, degree, "sine")
    sine_coeffs_op = Trans@dof_vals

    #evaluate the sine base approximation of the hat function
    tplot = np.linspace(0, T, 100)
    f_approx = evaluate_sine_coefficients(tplot, sine_coeffs_op, T)
