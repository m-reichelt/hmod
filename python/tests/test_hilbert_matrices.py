...
def test_sum():
    ...
    pol_deg_test = 0
    diagonal_index = 1
    result_first = hm.first_branch_sum(diagonal_index, nt, pol_deg_trial, pol_deg_test)
    result_second = hm.second_branch_sum(diagonal_index, nt, pol_deg_trial, pol_deg_test)
...
def test_kernel_matrix():
    ...
    K_op = Op.K
    K_opD = (2./nt**2)*K_op.toarray() #we need a factor of 2 since 0.5 is already in old implemenation
    Diff = Kd - K_opD
