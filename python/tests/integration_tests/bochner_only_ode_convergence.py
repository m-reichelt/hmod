...
def solve_ode_only_bochner_directly(nt : int, polynomial_degree : int, number_of_modes : int, T : float):
    """Solve a simple ODE problem $u'(t) +\mu u = f(t), u(0) = 0$ using only the Hilbert transformation as partial isometry.
    ...
    A = sm.get_lagrange_lagrange_matrix_for_derivatives(polynomial_degree_trial=polynomial_degree, polynomial_degree_test=polynomial_degree,
                                                        derivatives_trial=1, derivatives_test=0, nt=nt, T=T)
    M = sm.get_lagrange_lagrange_matrix_for_derivatives(polynomial_degree_trial=polynomial_degree, polynomial_degree_test=polynomial_degree,
                                                        derivatives_trial=0, derivatives_test=0, nt=nt, T=T)
    F = sm.get_legendre_lagrange_matrix_for_derivatives(polynomial_degree_trial=polynomial_degree_rhs, polynomial_degree_test=polynomial_degree,
                                                        derivatives_trial=0, derivatives_test=0, nt=nt, T=T)
...
def test_bochner_only_ode_convergence():
    T = 1.0
    ...
    for (p,n) in zip(polynomial_degrees, nt_values):
        l2e0, h12e_0 = solve_ode_only_bochner_directly(n, p, number_of_modes, T)
        l2e1, h12e_1 = solve_ode_only_bochner_directly(n*2, p, number_of_modes, T)
        ...
        tol = 0.1
        #the convergence rates only work for odd polynomial degree. This is strange, but maybe due to non-stability!!!
